...
\rpResumeAuto{
Pour notre projet de troisième année à l'ISIMA, nous avons choisi de réalisé une étude sur le protocole \keyword{6LoWPAN}.
C'est l'acronyme d'IPv6 over Low power Wireless Personal Area Network.
L'ère de l'Internet des Objets (\keyword{IdO}) a commencé : d'ici 2020, il y aura plus de 20 milliards d'objets connectés à travers le monde et la plus part d'entre eu fonctionneront sur piles ou batteries. Mais à l'inverse des autres protocoles qui ne sont pas d'une très grande efficacité énergétique, 6LowPAN a pour but d'amener IPv6 aux appareils qui ont une faible emprunte énergétique.

La première partie de l'étude était la découverte du protocole en lui-même, nous avons parcourus beaucoup de documentation et de spécification pour essayer de comprendre les mécanismes derrière le nom.
Puis il nous a été procuré des appareils (TI cc2538dk) pour créer un prototype, la première étape étant l'établissement d'une communications entre deux équipements.
L'utilisation de \keyword{Contiki}, un système d'exploitation léger, nous a permis d'utiliser 6LoWPAN.
Finalement, nous avons a utiliser encore plus d'appareils (Arduino Mega avec des Xbee Shield) pour tester l'\keyword{interopérabilité} des appareils, car l'une des forces de 6LoWPAN est que l'on peut ajouter, enlever ou remplacer un équipement dans le réseau sans perte de connectivité (à part une petite, le temps que les nœuds se resynchronisent). 
...
Actuellement, notre prototype (2 cc2538) marche correctement mais nous n'arrivons pas a tester l'interopérabilité car nous rencontrons des problèmes avec les Arduino.
Malheureusement nous sommes à court de temps, donc nous avons améliorer le prototype et préparons le terrain pour un projet de l'année prochaine sur le même sujet.
 
6LoWPAN est réellement un protocole plein de promesses mais pas très bien documenté, ce qui a rendue l'étude plutôt compliqué.
}{
